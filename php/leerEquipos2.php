    ...
    function get_equipos_rows($categoria_id){
      ...
      switch ($categoria_id) {
        ...
        case 'obtenerId':
          ...
          $rows_equipos = $conn->query("SELECT cp_id,cp_nombre FROM tb_perfil_cont_cfg where cp_cat_id = '".$idcat."' ");
          $numeroEquipos = 0;
          $myArray = [];
        ...
        case 'leerDatos':
          ...
          print_r($catnombre);
          $conn = crearConexion();
          $result = $conn->query("SELECT cp_id FROM tb_perfil_cont_cfg where cp_nombre = '".$contnombre."' ");
          $row = $result->fetch_array();
          //$idcat = $row['cp_id'];
